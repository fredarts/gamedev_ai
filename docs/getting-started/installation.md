...
Bem-vindo à documentação oficial do **Gamedev AI**! Esta página vai focar em como adicionar rapidamente o assistente ao seu projeto atual do Godot 4.6.

## Requisitos
* **Godot Engine:** Versão 4.6 ou superior (suporte nativo ao `.gdextension` não é necessário, o plugin roda puro em GDScript).
* Conta Google ou OpenRouter para chaves de API.

## Passo a Passo Manual (GitHub)

1. Crie ou abra um projeto existente no Godot 4.6.
...
```
*(Ou baixe o arquivo `.zip` da aba de Releases no GitHub e extraia dentro de `res:addons/gamedev_ai`).*
...
Pronto! Ao ativá-lo pela primeira vez, você verá o painel de Chat do Gamedev AI aparecer na *Dock* direita do seu editor.
