...
| 开关 | 功能 |
|--------|--------|
| **Context** | 激活：插件会自动将当前编辑器中打开的脚本完整内容附加到发送的消息中。 |
| **Screenshot** | 激活：自动截取 Godot 窗口并将其发送给 AI 进行视觉分析。 |
| **Plan First** | 激活：AI 不编写代码，只回复详细的 Markdown 计划。审查后点击 "Execute Plan"。 |
| **Watch Mode** | 激活：AI 自动监控 Godot 输出控制台。遇到严重（红色）错误时，它会自动提议修复。冷却时间：30秒，限制：每周期 3 次修复。 |
